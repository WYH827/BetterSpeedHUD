...
一个用于 Minecraft Forge 的速度 HUD 模组：在载具或鞘翅滑翔时，于屏幕右下角显示速度、档位与速度条，界面风格类似地平线（Horizon）系列游戏的抬头显示，并且所有元素都可以在游戏内自由调整。
...
- **Horizon 风格布局**：左侧 `D` 档、中间三位大号速度数字（前导 0 灰色）、右侧 `km/h`、下方速度条。
...
> 注意：mods 文件夹中不要同时放入多个版本的 BetterSpeedHUD jar（modId 相同，Forge 只会加载其中一个）。
